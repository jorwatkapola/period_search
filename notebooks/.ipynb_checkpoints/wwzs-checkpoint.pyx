...
#         cnp.ndarray magnitudes,
#         int time_divisions,
#         cnp.ndarray freq_params,
#         float decay_constant

    # WWT Stars Here
cpdef tau_loop(float dtau, cnp.ndarray freq, cnp.ndarray timestamps, cnp.ndarray magnitudes, float decay_constant):
    """
    ...
    cdef int index = 0
    cdef cnp.ndarray output = np.empty((len(freq), 6))
    cdef int nstart = 1
    cdef float dvarw = 0.0

    cdef float dz, dweight, dweight2, domega, cos_dz, sin_dz
    cdef float dneff, davew, dpowz, damp, dpower
    cdef cnp.ndarray dcoef
    cdef cnp.ndarray dvec = np.empty(3)
    cdef cnp.ndarray dmat = np.empty([3, 3])
#         cdef list non_zero_dats

    cdef int numdat = len(timestamps)


    #index for loop
    cdef Py_ssize_t idat
...
#             non_zero_dats = []
        ...
        for idat in range(nstart, numdat):
            ...
            dz = domega * (timestamps[idat] - dtau)
            dweight = exp(-1 * decay_constant * dz * dz)
...
#                     non_zero_dats.append(timestamps[idat])
                ...
                dmat[1, 1] += dweight * cos_dz * cos_dz
                dmat[1, 2] += dweight * cos_dz * sin_dz
                dmat[2, 2] += dweight * sin_dz * sin_dz
        ...
        if dneff > 3:
            ...
            dpowz = (dneff - 3.0) * dpower / (2.0 * (dvarw - dpower))  # WWZ eq. 5-12
            damp = sqrt(dcoef[1] ** 2 + dcoef[2] ** 2)  # WWA eq. 5-14
        else:
            dpowz = 0.0
            damp = 0.0
...
#             samp_med = cnp.median(cnp.diff(cnp.array(non_zero_dats)))
...
cpdef wwt(cnp.ndarray timestamps, 
        cnp.ndarray magnitudes,
        int time_divisions,
        cnp.ndarray freq_params,
        float decay_constant,
...
#         parallel: bool = True
    ...
    #define cython variables

    # Get taus to compute WWZ (referred in paper as "time shift(s)")
    
    cdef cnp.ndarray tau = np.linspace(timestamps[0], timestamps[-1], time_divisions)
    cdef int ntau = len(tau)

    # Calculate pseudo sample rate and largest time window to check for requirements
    cdef float freq_pseudo_sr = 1 / np.median(np.diff(timestamps))  # 1 / median period

    # noinspection PyArgumentList
    cdef float largest_tau_window = tau[1] - tau[0]
    ...
    cdef cnp.ndarray freq = freq_params
    cdef int nfreq = len(freq)
...
#     num_cores = multiprocessing.cpu_count()
    ...
    cdef float dtau
    cdef cnp.ndarray output = np.empty((ntau, nfreq, 6))
    cdef cnp.ndarray tau_mat, freq_mat, wwz_mat, amp_mat, dcoef_mat, dneff_mat
#     if parallel:
#         print("parallel is a no no")
#     else:
    for i, dtau in enumerate(tau):
        output[i] = tau_loop(dtau, freq, timestamps, magnitudes, decay_constant)
...
#     samp_med_mat: cnp.ndarray = output[:, :, 6].reshape([ntau, nfreq])

#     output = cnp.array([tau_mat, freq_mat, wwz_mat, amp_mat, dcoef_mat, dneff_mat, samp_med_mat])
#     output = [tau_mat, freq_mat, wwz_mat, amp_mat, dcoef_mat, dneff_mat]
    ...
    return np.array((tau_mat, freq_mat, wwz_mat, amp_mat, dcoef_mat, dneff_mat))