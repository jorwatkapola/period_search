...
#         dvarw_list = [] ###del
        ...
        for dfreq in freq:
            ...
            dz = torch.tensor(domega * (timestamps - dtau))
...
#             if dweight > 10 ** -9
            ...
            dweight2 = torch.sum(dweight ** 2)
            dvarw = torch.sum(dweight * magnitudes ** 2)
            dmat[0, 0] = torch.sum(dweight)
            dmat[0, 1] = torch.sum(dweight * cos_dz)
            dmat[0, 2] = torch.sum(dweight * sin_dz)
            dmat[1, 1] = torch.sum(dweight * cos_dz ** 2)
            dmat[1, 2] = torch.sum(dweight * cos_dz * sin_dz)
            dmat[2, 2] = torch.sum(dweight * sin_dz ** 2)
            
            # parallel to the 3 trial functions (5-5, 6, 7)
            dvec[0] = torch.sum(dweight * magnitudes)
            dvec[1] = torch.sum(dweight * magnitudes * cos_dz)
            dvec[2] = torch.sum(dweight * magnitudes * sin_dz)
...
#                     print("determinant is zero, using pseudo-inverse.")
#                 else:
                dmat = torch.pinverse(dmat)

                # set dcoef and dpower
                dcoef = torch.matmul(dmat, dvec)  # y1, y2, and y3 from eq. 4-4, with 5-5, 6, 7
                dpower = torch.dot(dcoef, dvec) - (davew ** 2)  # weighted model function eq. 5-10

                dpowz = (dneff - 3.0) * dpower / (2.0 * (dvarw - dpower))  # WWZ eq. 5-12
                damp = torch.sqrt(dcoef[1] ** 2 + dcoef[2] ** 2)  # WWA eq. 5-14
            else:
                dpowz = 0.0
                damp = 0.0
